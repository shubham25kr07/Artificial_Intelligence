...
int count_neighbour(vp_ &classrooms,int k,int m,int n,int row,int col,int classes,string batch)
{
    int cnt=0;
    if(row-1>=0 && classrooms[classes][row-1][col].first=="00")
    {
        if(issafe(classrooms,batch,k,m,n,row-1,col,classes))
            cnt++;
    }
    if(row+1<m && classrooms[classes][row+1][col].first=="00")
    {
    ...
    if(col-1>=0 && classrooms[classes][row][col-1].first=="00")
    {
        if(issafe(classrooms,batch,k,m,n,row,col-1,classes))
            cnt++;
    }
    if(col+1<n && classrooms[classes][row][col+1].first=="00")
    {
    ...
    if(row-1>=0 && col-1>=0 &&  classrooms[classes][row-1][col-1].first=="00")
    {
        if(issafe(classrooms,batch,k,m,n,row-1,col-1,classes))
            cnt++;
    }
    if(row-1>=0 && col+1<n &&  classrooms[classes][row-1][col+1].first=="00")
    {
...
vector<pair<int,string> >  least_constraining_value(vp_ &classrooms,mv_ &sdata,m_ &visited,int k,int m,int n,int row,int col,int classes)
{
    ...
    for(auto it=sdata.begin();it!=sdata.end();it++)
    {
        ...
        if(issafe(classrooms,batch,k,m,n,row,col,classes))
        {
            ...
            int neigh_cnt=count_neighbour(classrooms,k,m,n,row,col,classes,batch);                          int tot_cnt=cnt*neigh_cnt;

            vv.push_back(make_pair(tot_cnt,batch));
        }
    }
...
bool solve(vp_ &classrooms,mv_&sdata,m_ &visited,int k,int m,int n,int sitted)
{
    ...
    vector<pair<int,string> > temp_batch;

    temp_batch=least_constraining_value(classrooms,sdata,visited,k,m,n,row,col,classes);

    for(auto it=temp_batch.begin();it!=temp_batch.end();it++)
    {
        string batch=(*it).second;
        if(issafe(classrooms,batch,k,m,n,row,col,classes))
        {
            //vector<string> v=it->second;
            vector<string> v=sdata[batch];
